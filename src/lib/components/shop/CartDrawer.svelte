	...
	<DrawerContent>
		<div class="mx-auto w-full max-w-xl">
			<DrawerHeader class="text-left">
				<DrawerTitle class="relative font-serif text-xl font-light">
					<span>Your Cart</span>
					<span class="absolute right-0 text-xs text-muted-foreground">
						<Spinner
							class="transition-opacity duration-300 {loading ? 'opacity-100' : 'opacity-0'}"
						/>
					</span>
				</DrawerTitle>
			</DrawerHeader>
			<div class="px-4 pb-4">
				...
				{:else}
					...
					<DrawerFooter class="px-0">
						...
						<p class="text-xs text-muted-foreground/60">Shipping calculated at checkout</p>
						<Button
							class="w-full rounded-none text-sm tracking-wide"
							href={cart.checkoutUrl}
							disabled={loading}
							aria-disabled={loading}>Proceed to Checkout</Button
						>

						<div class="flex items-center gap-4 py-2">
							<Separator orientation="horizontal" class="flex-1" />
							<p class="text-xs text-muted-foreground/60">or</p>
							<Separator orientation="horizontal" class="flex-1" />
						</div>

						<Button class="w-full rounded-none text-sm tracking-wide" href="/shop" variant="outline"
