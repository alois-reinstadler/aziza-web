document.addEventListener('DOMContentLoaded', function () {
	...
	const navBottomLinksSplits = Array.from(
		navBottomLinks,
		(n) => new SplitType(n, { type: 'chars' })
	);

	const navTopLinksSplits = Array.from(navTopLinks, (n) => new SplitType(n, { type: 'chars' }));
	...
	const openAnimation = gsap
		...
		.from(
			...
			{
				clipPath: (index) =>
					index % 2 === 1 ? 'inset(1px 100% 1.15px 0%)' : 'inset(1px 0% 1.15px 100%)',
				duration: 1.25,
				ease: 'power3.inOut'
			},
			0.25
